...
assessment_examples:
  '0': |
      - Architecture docs changed directly on main with no review or tracking.
      - No decision references in change history; rationale missing.
      - No consistent version history for diagrams (binary overwrites).
    Governance absent; high risk of silent regressions.
      - Some PRs used but self-merged commonly; sparse review comments.
      - Changelog or release notes missing / irregular.
      - Large multi-topic changes lumped into single commits.
    Traceability partial and unreliable.
  '2': |
      - Regular PR flow established; review happens but not enforced (occasional direct pushes).
      - Decision linkage ad-hoc (references appear inconsistently).
      - Diagram source sometimes included; others remain binary only.
    Rationale present sporadically.
      - Peer review required for most changes; template partially adopted.
      - Decision IDs linked in majority of substantive PRs.
      - Changelog maintained but selective omissions exist.
      - Some automation (lint / link check) warns but does not block.
    Solid baseline; enforcement not uniform.
  '4': |
      - All substantive changes via PR with at least one reviewer + rationale section.
      - Automatic link / metadata validation gating merges.
      - Changelog auto-generated; manual curation for highlights.
      - Diagram-as-code >90%; binary legacy tracked for migration.
    Minor edge cases (small doc fixes) occasionally bypass template.
      - Full automation: schema / link / decision linkage / diagram source checks required to pass.
      - Every decision-affecting PR references ADR(s) & triggers diagram diff preview.
      - Changelog & dependency graph updated automatically post-merge.
      - Audit trail complete; no direct main merges.
    Governance optimised with low friction and high assurance.
