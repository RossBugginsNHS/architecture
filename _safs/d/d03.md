...
assessment_examples:
  '0': |
      - No explicit freshness metadata; cannot prove when any artefact was last verified.
      - Stale content evident (references to deprecated components) with no review plan.
      - Removal of outdated docs via deletion (no archival) losing rationale history.
    Ad-hoc updates only when issues arise.
      - Ad-hoc “last updated” timestamps present but mix creation edits and verification.
      - Only a few critical documents manually reviewed recently; rest untracked.
      - No owner assignment; updates reliant on volunteer effort.
    Visibility minimal; risk of widespread silent drift.
  '2': |
      - Introduced last_verified field for some artefacts (<40%).
      - Informal spreadsheet lists a subset of review cadences; out-of-date entries present.
      - No automation; manual spot checks triggered occasionally.
    Intent shown but insufficient coverage & sustainability.
      - Majority (~60–70%) of artefacts have last_verified & next_review_due.
      - Overdue items identifiable but remediation backlog growing.
      - Manual script run intermittently to highlight stale docs.
      - Archived (superseded) items kept, though classification inconsistent.
    Working baseline; needs automation & tighter SLA enforcement.
  '4': |
      - High coverage (≥90%) of freshness metadata; dashboard summarises stale %, trend decreasing.
      - Automated job flags overdue items and opens issues with owners.
      - Archival workflow consistent (superseded flag + link forward/backward).
      - Review cadence aligned with risk/criticality tiers.
    Only small pockets of drift awaiting scheduled review.
      - 100% artefacts have machine-validated metadata; CI fails missing or stale beyond grace period.
      - Freshness, coverage and mean days overdue metrics published; continuous reduction tracked.
      - Automated PR suggestions for “review-only” refresh (metadata bump with diff context) after content diff check.
      - Deletion replaced by state transitions (active → superseded → archived) preserving trace.
    Continuous improvement cycle demonstrably embedded.
