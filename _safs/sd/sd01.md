...
assessment_examples:
  '0': |
    - No articulated methodology; design emerges from ad-hoc implementation.
    - No domain context mapping or threat model.
      - Sporadic modelling sessions; minimal artefacts (single outdated diagram).
      - Security & service considerations appended late.
  '2': |
    - Initial bounded context map & a few ADRs exist; threat model partial.
    - Method usage inconsistent across teams.
      - Regular modelling cadence; ADRs reference domain & security rationale.
      - Threat model current; some terminology drift detected.
  '4': |
    - Methodology embedded in delivery workflow; artefacts current & referenced.
    - Domain language consistent across repos & contracts.
      - Continuous validation (linting of context names, threat model diffs in PRs).
      - Feedback loops from production events refine domain model proactively.
